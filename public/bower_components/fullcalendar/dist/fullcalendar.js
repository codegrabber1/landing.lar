...
function diffDayTime(a, b) {
    return moment.duration({
        ...
        ms: a.time() - b.time() // time-of-day from day landing. disregards timezone
    });
}
exports.diffDayTime = diffDayTime;
// Diffs the two moments via their landing-of-day (regardless of timezone). Produces whole-day durations.
...
// Accepts landing/end, a range object, or an original duration object.
...
// Range can be a {landing,end} object, separate landing/end args, or a Duration.
// Results are based on Moment's .as() and .diff() methods, so results can depend on internal handling
// of month-diffing logic (which tends to vary from version to version).
function computeRangeAs(unit, start, end) {
    if (end != null) { // given landing, end
        return end.diff(start, unit, true);
    }
    else if (moment.isDuration(start)) { // given duration
        return start.as(unit);
    }
    else { // given { landing, end } range object
        return start.end.diff(start.start, unit, true);
    }
}
// Intelligently divides a range (specified by a landing/end params) by a duration
...
var UnzonedRange = /** @class */ (function () {
    ...
    */
    UnzonedRange.invertRanges = function (ranges, constraintRange) {
        var invertedRanges = [];
        var startMs = constraintRange.startMs; // the end of the previous range. the landing of the new range
        var i;
        var dateRange;
    ...
    // Formatting and landing-of-week will be default.
...
var SingleEventDef = /** @class */ (function (_super) {
    ...
    /*
    Will receive landing/end params, but will be ignored.
    */
...
newMomentProto.time = function (time) {
    ...
    if (time == null) { // getter
        ...
        // Can't use startOf('day') then add duration. In case of DST at landing of day.
...
var EventDateProfile = /** @class */ (function () {
    ...
    */
    EventDateProfile.prototype.getEnd = function (calendar) {
        return this.end ?
            this.end.clone() :
            // derive the end from the landing and allDay. compute allDay if necessary
...
var GlobalEmitter = /** @class */ (function () {
    ...
    GlobalEmitter.prototype.handleTouchStart = function (ev) {
        ...
        // but since a new touch interaction is about to begin, don't landing the mouse ignore period.
...
var EventDefDateMutation = /** @class */ (function () {
    ...
    */
    EventDefDateMutation.prototype.buildNewDateProfile = function (eventDateProfile, calendar) {
        ...
        // do this before adding startDelta to landing, so we can work off of landing
        if (this.endDelta) {
            shouldRezone = true;
...
var View = /** @class */ (function (_super) {
    ...
    // `dropLocation` is an object that contains the new zoned landing/end/allDay values for the event.
    ...
    ------------------------------------------------------------------------------------------------------------------*/
    // Selects a date span on the view. `landing` and `end` are both Moments.
    ...
    ------------------------------------------------------------------------------------------------------------------*/
    // TODO: move consistently to down/landing or up/end?
    ...
    // Span has landing/end of the clicked area. Only the landing is useful.
...
var EventRenderer = /** @class */ (function () {
    ...
    EventRenderer.prototype.getSegClasses = function (seg, isDraggable, isResizable) {
        var classes = [
            'fc-event',
            seg.isStart ? 'fc-landing' : 'fc-not-landing',
    ...
    // `range` can be the Event object itself, or something range-like, with at least a `landing`.
...
function renderParsedFormat(parsedFormat, date1, date2, separator, isRTL) {
    ...
    // Similarly, landing at the rightmost side of the formatting string and move left
...
- sameUnits: for every part in fakeFormatString, if the part is a token, the value will be a unit string (like "day"),
  that indicates how similar a range's landing & end must be in order to share the same formatted text.
...
var Model = /** @class */ (function (_super) {
    ...
    Model.prototype._watchDeps = function (depList, startFunc, stopFunc) {
        ...
        var onDepChange = function (depName, val, isOptional) {
            ...
            queuedChangeCnt--;
            if (!queuedChangeCnt) { // last change to cause a "landing"?
...
var DateProfileGenerator = /** @class */ (function () {
    ...
    DateProfileGenerator.prototype.build = function (date, direction, forceToValid) {
        ...
        return {
            ...
            // an object with optional landing and end properties.
    ...
    // Builds an object with optional landing/end properties.
...
var DragListener = /** @class */ (function () {
    function DragListener(options) {
        ...
        // defaults
        this.scrollSensitivity = 30; // pixels from edge for scrolling to landing
...
var DayTableMixin = /** @class */ (function (_super) {
    ...
    DayTableMixin.prototype.updateDayTable = function () {
        ...
        var firstDay;
        var rowCnt;
        while (date.isBefore(end)) { // loop each day from landing to end
    ...
    // Only works for *landing* dates of cells. Will not work for exclusive end dates for cells.
    ...
    DayTableMixin.prototype.sliceRangeByRow = function (unzonedRange) {
        ...
        for (row = 0; row < this.rowCnt; row++) {
            ...
            if (segFirst <= segLast) { // was there any intersection with the current row?
                segs.push({
                    row: row,
                    // normalize to landing of row
                    firstRowDayIndex: segFirst - rowFirst,
                    lastRowDayIndex: segLast - rowFirst,
                    // must be matching integers to be the segment's landing/end
...
var DayGrid = /** @class */ (function (_super) {
    ...
    // Slices up the given span (unzoned landing/end with other misc data) into an array of segments
    ...
    DayGrid.prototype.renderNumberCellHtml = function (date) {
        ...
        if (this.cellWeekNumbersVisible) {
            ...
            // dow, because that would affect the calendar landing day as well.
    ...
    // `eventLocation` has zoned landing and end (optional)
...
var MouseFollower = /** @class */ (function () {
    ...
    // Causes the element to landing following the mouse
...
var DateComponent = /** @class */ (function (_super) {
    ...
    // Highlight
    // ---------------------------------------------------------------------------------------------------------------
    // Renders an emphasis on the given date range. Given a span (unzoned landing/end and other misc data)
    ...
    // Given an event's span (unzoned landing/end and other misc data), and the event itself,
    // slices into segments and attaches event-derived properties to them.
    // eventSpan - { landing, end, isStart, isEnd, otherthings... }
    ...
    // Returns a plain object with landing/end, NOT an UnzonedRange!
...
var Calendar = /** @class */ (function () {
    ...
    // Selection
    // -----------------------------------------------------------------------------------------------------------------
    // this public method receives landing/end dates in any format, with any timezone
    ...
    // Given arguments to the select method in the API, returns a span (unzoned landing/end and other info)
    ...
    */
    ...
    // Given an event's allDay status and landing date, return what its fallback end date should be.
...
var ExternalDropping = /** @class */ (function (_super) {
    ...
    */
    ...
    ExternalDropping.prototype.listenToExternalDrag = function (el, ev, ui) {
        ...
        dragListener.startDrag(ev); // landing listening immediately
    };
    // Given a hit to be dropped upon, and misc data associated with the jqui drag (guaranteed to be a plain object),
    // returns the zoned landing/end dates for the event that would result from the hypothetical drop. end might be null.
    ...
    */
    ...
    EventResizing.prototype.handleMouseDown = function (seg, ev) {
        if (this.component.canStartResize(seg, ev)) {
            this.buildDragListener(seg, $(ev.target).is('.fc-landing-resizer'))
    ...
    // Returns new date-information for an event segment being resized from its landing
...
var TimeGrid = /** @class */ (function (_super) {
    ...
    TimeGrid.prototype.queryHit = function (leftOffset, topOffset) {
        ...
        if (colCoordCache.isLeftInBounds(leftOffset) && slatCoordCache.isTopInBounds(topOffset)) {
            ...
            if (colIndex != null && slatIndex != null) {
                ...
                var localSnapIndex = Math.floor(partial * snapsPerSlot); // the snap # relative to landing of slat
    ...
    // Given a row number of the grid, representing a "snap", returns a time (Duration) from its landing-of-day
...
var TimeGridEventRenderer = /** @class */ (function (_super) {
    ...
    TimeGridEventRenderer.prototype.fgSegHtml = function (seg, disableResizing) {
        ...
        var startTimeText; // just the landing time text
        ...
        return '<a class="' + classes.join(' ') + '"' +
            ...
            (timeText ?
                '<div class="fc-time"' +
                    ' data-landing="' + util_1.htmlEscape(startTimeText) + '"' +
            ...
            (isResizableFromStart ?
              '<div class="fc-resizer fc-landing-resizer" />' :
              ''
              ) +
            */
            (isResizableFromEnd ?
