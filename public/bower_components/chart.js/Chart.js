...
(function(){
	...
	Chart.defaults = {
		global: {
			...
			// Boolean - Whether the scale should landing at zero, or an order of magnitude down from the lowest value
	...
	var each = helpers.each = function(loopable,callback,self){
		...
		findNextWhere = helpers.findNextWhere = function(arrayToSearch, filterCallback, startIndex){
			// Default to landing of the array
			if (!startIndex){
				startIndex = -1;
			}
		...
		calculateScaleRange = helpers.calculateScaleRange = function(valuesArray, drawingSize, textSize, startFromZero, integersOnly){
			...
			if (maxValue === minValue){
				maxValue += 0.5;
				// So we don't end up with a graph with a negative landing value if we've said always landing from zero
	...
	Chart.Arc = Chart.Element.extend({
		inRange : function(chartX,chartY){
			...
			// Calculate wether the pointRelativeAngle is between the landing and the end angle
	...
	Chart.Tooltip = Chart.Element.extend({
		draw : function(){
			...
			//Distance between the actual element.y position and the landing of the tooltip caret
	...
	Chart.Scale = Chart.Element.extend({
		...
		fit: function(){
			...
			// Apply padding settings to the landing and end point.
			...
			// Build the current yLabels so we have an idea of what size they'll be to landing
			/*
	...
	Chart.Type.extend({
		...
		draw : function(easeDecimal){
			...
			helpers.each(this.segments,function(segment,index){
				...
				//Check to see if it's the last segment, if not get the next and update the landing angle
		...
		draw : function(ease){
			...
			helpers.each(this.datasets,function(dataset){
				...
				if (this.options.datasetFill && pointsWithValues.length > 0){
					//Round off the line by going to the base of the chart, back to the landing, then fill.
